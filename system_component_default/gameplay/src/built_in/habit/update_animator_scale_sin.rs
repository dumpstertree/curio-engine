use crate::{
    built_in::facet::{animator::animator_scale_sin::AnimatorScaleSin, transform::transform2d::Transform2D},
...
impl Habit for Instance {
    fn tick(&mut self, game_state: &mut Ledger, context3d: &mut Context3D, _: &mut EventQueue) {
        ...
        context3d.edit::<(&AnimatorScaleSin, &mut Transform2D)>(|x| {
            //
            for (_, (animator, transform2d)) in x {
                ...
                if animator.enabled() {
                    ...
                    transform2d.scale = target;
                } else {
                    let max = animator.max();
                    let min = animator.min();
                    transform2d.scale = min + (max - min) / 2.0;
                }
            }
        });
    }
