...
impl PluginCommon for SystemComponentDefaultGraphics {
    ...
    fn render(&mut self, output_texture: &Texture, output_view: &TextureView, mut encoder: &mut egui_wgpu::wgpu::CommandEncoder, ledger: &mut Vec<Ledger>, _: &mut Vec<Nerve>) {
        ...
        if let Some(capture_tex) = s.gpu.capture_texture() {
            ...
            encoder.copy_texture_to_texture(
                egui_wgpu::wgpu::TexelCopyTextureInfo {
                    texture: &output_texture, // ← swapchain, not offscreen
                    mip_level: 0,
...
impl SystemComponentDefaultGraphics {
    ...
    pub fn generate_render_texture() -> (egui_wgpu::wgpu::Texture, TextureView) {
        ...
        let offscreen_texture = device.create_texture(&egui_wgpu::wgpu::TextureDescriptor {
            ...
            size: egui_wgpu::wgpu::Extent3d { width: 1280, height: 720, depth_or_array_layers: 1 },
            mip_level_count: 1,
            sample_count: 1,
            dimension: egui_wgpu::wgpu::TextureDimension::D2,
            format: egui_wgpu::wgpu::TextureFormat::Rgba8UnormSrgb,
            usage: egui_wgpu::wgpu::TextureUsages::RENDER_ATTACHMENT | egui_wgpu::wgpu::TextureUsages::TEXTURE_BINDING | egui_wgpu::wgpu::TextureUsages::COPY_SRC,
            view_formats: &[],
        });
