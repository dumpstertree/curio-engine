use crate::{ai_resolver::CardEvents, game_events::GameEvents, state::state_ball_mode::StateBallMode};
use core::{
...
impl ecs_event_reciever::EventReciever<GameEvents> for EventReciever {
    fn dequeue_event(&mut self, game_state: &mut GameState, _: &mut World, _: &mut EventQueue, event: &GameEvents) {
        match event {
            GameEvents::ApplyCardAttributeEventSetBallMode(ball_mode) => {
...
impl EventReciever {
    pub fn recieve(event: &CardEvents, game_state: &mut GameState) -> Vec<CardEvents> {
        match event {
            CardEvents::ApplyEventSetBallMode(ball_mode) => {
                game_state.edit::<StateBallMode>(|x| {
                    x.mode = ball_mode.clone();
                });
            }
