...
// use crate::io::asset_loader::{ASSET_UID_SHADER_MODULE_LIT, ASSET_UID_SHADER_MODULE_UNLIT};
// use crate::io::file::File;
// use crate::{GPUInstance, TextureAsset};
...
//     }
// }
// rendering/src/system_gpu.rs
use crate::{
    engine_services::services,
    io::{
        asset_loader::{ASSET_UID_SHADER_MODULE_LIT, ASSET_UID_SHADER_MODULE_UNLIT},
        file::File,
    },
    GPUInstance, TextureAsset,
};
use egui_wgpu::wgpu::{Adapter, Device, Queue, ShaderModule, Surface, SurfaceConfiguration};
use std::{
    collections::HashMap,
...
// NO MORE static — this is now owned by whoever creates it
pub struct SystemGPU {
...
impl SystemGPU {
    pub fn new(gpu_instance: Arc<GPUInstance>) -> Self {
        // let depth_texture = Arc::new(TextureAsset::create_depth_texture("depth_texture"));

        let surface = gpu_instance.surface.clone();
        let device = gpu_instance.device.clone();
        let config = gpu_instance.config.clone();
        surface.configure(&*device, &*config);

        Self {
            device,
            queue: gpu_instance.queue.clone(),
            surface,
            adapter: gpu_instance.adapter.clone(),
            window: gpu_instance.window.clone(),
            // depth_texture,
            config,
        }
    }

    // all your existing get_device(), get_queue() etc become
    // regular methods on self instead of statics
