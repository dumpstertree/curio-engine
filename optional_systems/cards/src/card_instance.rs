...
use curio_core::{collections::ledger::Ledger, random::Random};
...
impl CardInstance {
    ...
    pub fn get_cost(&self, ledger: &Ledger, user_id: i32) -> i32 {
        self.get_statement(ledger, user_id).cost
    }
    pub fn get_attributes_events(&self, ledger: &Ledger, user_id: i32) -> Vec<CardAttributeEvents> {
        self.get_statement(ledger, user_id).events.clone()
    }
    pub fn get_attributes_modifiers(&self, ledger: &Ledger, user_id: i32) -> Vec<CardAttributeModifiers> {
        self.get_statement(ledger, user_id).attributes.clone()
    }
    pub fn get_attributes_requirements(&self, ledger: &Ledger, user_id: i32) -> Vec<CardAttributeRequirement> {
        self.get_statement(ledger, user_id).requirements.clone()
    }
    ...
    pub fn get_statement(&self, ledger: &Ledger, user_id: i32) -> CardStatement {
    ...
    pub fn has_statement(&self, ledger: &Ledger, user_id: i32) -> bool {
        let master = self.get_master();
        for statement in &master.statements {
            let is_met = statement
                .requirements
